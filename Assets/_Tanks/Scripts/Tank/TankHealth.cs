...
namespace Tanks.Complete
{
    public class TankHealth : MonoBehaviour
    {
        ...
        public ParticleSystem[] m_DamageParticles;          // Array of particle systems to play when the tank is damaged.
        ...
        private void SetHealthUI ()
        {
            ...
            // Update damage particles based on health
            if (m_DamageParticles != null && m_DamageParticles.Length > 0)
            {
                float healthPercent = m_CurrentHealth / m_StartingHealth;
                
                for (int i = 0; i < m_DamageParticles.Length; i++)
                {
                        // Control all particle systems in children (for complex effects like Gabriel Aguiar's)
                        ParticleSystem[] allChildren = m_DamageParticles[i].GetComponentsInChildren<ParticleSystem>();
                        
                        foreach (ParticleSystem ps in allChildren)
                        {
                            ...
                            if (healthPercent < 0.5f)
                            {
                                ...
                                // Ratio: 0 at 50% health, 1.0 at 0% health
                                float ratio = (0.5f - healthPercent) / 0.5f;
                                float intensity = ratio;

                                // Check the name of the particle system to apply different logic
                                string psName = ps.name.ToLower();
                                if (psName.Contains("impact") || psName.Contains("particles") || psName.Contains("brighthold") || psName.Contains("darkback"))
                                {
                                    intensity = ratio * ratio;
                                }
                                else if (psName.Contains("smoke"))
                                {
                                    intensity = ratio * 15.0f; 
                                }

                                emission.rateOverTimeMultiplier = Mathf.Clamp(intensity, 0f, 5f);
