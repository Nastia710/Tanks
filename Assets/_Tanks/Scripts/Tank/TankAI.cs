...
namespace Tanks.Complete
{
    ...
    public class TankAI : MonoBehaviour
    {
        ...
        private Vector3 m_StuckLastPosition;            
        private void Awake()
        {
            ...
            m_StuckLastPosition = transform.position;
        }
        ...
        private void FixedUpdate()
        {
            ...
            if (m_IsMoving)
            {
                // Якщо за останній час ми просунулися менше ніж на 0.01 одиниці
                if (Vector3.Distance(rb.position, m_StuckLastPosition) < 0.01f)
                {
                ...
                {
                    m_StuckTimer = 0.0f;
                    m_StuckLastPosition = rb.position;
                }

                // Якщо стоїмо на місці більше 1.5 секунди — "панікуємо" (тікаємо вбік)
            ...
            // Пересування через фізичну швидкість
            if (m_IsMoving)
            {
                float moveSpeed = Mathf.Clamp01(orientDot) * m_Movement.m_Speed;
            ...
            // Обертання
            rotatingAngle = Mathf.Sign(rotatingAngle) * Mathf.Min(Mathf.Abs(rotatingAngle), m_Movement.m_TurnSpeed * Time.fixedDeltaTime);
